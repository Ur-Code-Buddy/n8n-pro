...
export class InstanceSettingsLoaderConfig {
	/**
	 * When true, creates a default instance owner on first boot if none exists.
	 * Set N8N_DEFAULT_OWNER_ENABLED=false to disable.
	 */
	@Env('N8N_DEFAULT_OWNER_ENABLED')
	defaultOwnerEnabled: boolean = true;

	@Env('N8N_DEFAULT_OWNER_EMAIL')
	defaultOwnerEmail: string = 'baivab@admin.local';

	@Env('N8N_DEFAULT_OWNER_PASSWORD')
	defaultOwnerPassword: string = 'admin@ccess4321';

	@Env('N8N_DEFAULT_OWNER_FIRST_NAME')
	defaultOwnerFirstName: string = 'Admin';

	@Env('N8N_DEFAULT_OWNER_LAST_NAME')
	defaultOwnerLastName: string = 'User';
