...
const onProjectSelected = (projectId: string) => {
	...
	if (Array.isArray(model.value)) {
		model.value = [
			...model.value,
			{ ...project, role: props.roles?.[0]?.slug ?? 'workflow:editor' },
		];
	} else {
		model.value = { ...project, role: props.roles?.[0]?.slug ?? 'workflow:editor' };
	}
...
const onRoleAction = (project: ProjectSharingData, role: string) => {
	...
	model.value = model.value.map((entry) => (entry.id === project.id ? { ...entry, role } : entry));
};
