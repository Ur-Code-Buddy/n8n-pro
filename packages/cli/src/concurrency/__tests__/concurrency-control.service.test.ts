...
describe('ConcurrencyControlService', () => {
	...
	// look set throughout the suite. The lazy-resolution tests toggle this
	// off in their own `beforeEach`.
	...
	describe('evaluation queue when env is unset (lazy)', () => {
		// Env unset → resolver returns unlimited (-1). The eval queue is
		// resolved lazily on first eval-mode throttle.
		beforeEach(() => {
		...
		it('resolves to unlimited on first throttle, creating no eval queue', async () => {
			globalConfig.executions.concurrency.evaluationLimit = -1;
			...
			// Unlimited (-1) means no cap is enforced, so no queue is built.
			// @ts-expect-error Private property
			expect(service.queues.get('evaluation')).toBeUndefined();
			// @ts-expect-error Private property
			expect(service.limits.get('evaluation')).toBe(-1);
		});
