...
describe('License', () => {
	...
	test('returns true for all features when unlockAllFeatures is set', async () => {
		(LicenseManager.prototype.hasFeatureEnabled as jest.Mock).mockReturnValue(false);

		const unlockedLicense = new License(
			mockLogger(),
			instanceSettings,
			mock(),
			mock(),
			mock<GlobalConfig>({
				license: { ...licenseConfig, unlockAllFeatures: true },
				multiMainSetup: { enabled: false },
			}),
		);
		await unlockedLicense.init();

		expect(unlockedLicense.isLicensed('feat:sharing')).toBe(true);
		expect(unlockedLicense.isLicensed('feat:ldap')).toBe(true);
		expect(unlockedLicense.isLicensed('feat:variables')).toBe(true);
		expect(unlockedLicense.isLicensed('feat:apiDisabled')).toBe(false);
		expect(unlockedLicense.isLicensed('feat:showNonProdBanner')).toBe(false);
	});

	test('returns unlimited quotas when unlockAllFeatures is set', () => {
		const unlockedLicense = new License(
			mockLogger(),
			instanceSettings,
			mock(),
		...
		expect(unlockedLicense.getValue('quota:maxTeamProjects')).toBe(-1);
		expect(unlockedLicense.getValue('quota:users')).toBe(-1);
		expect(unlockedLicense.getPlanName()).toBe('Enterprise');
	});

	test('hasFeatureInCert returns true for unlocked features', () => {
		(LicenseManager.prototype.hasFeatureEnabled as jest.Mock).mockReturnValue(false);
		...
		expect(unlockedLicense.hasFeatureInCert('feat:sharing')).toBe(true);
		expect(unlockedLicense.hasFeatureInCert('feat:apiDisabled')).toBe(false);
	});
