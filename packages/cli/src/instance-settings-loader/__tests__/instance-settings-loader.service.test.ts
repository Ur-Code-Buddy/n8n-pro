...
import type { DefaultOwnerInstanceSettingsLoader } from '../loaders/default-owner.instance-settings-loader';
...
describe('InstanceSettingsLoaderService', () => {
	...
	const defaultOwnerLoader = mock<DefaultOwnerInstanceSettingsLoader>();
	...
	beforeEach(() => {
		...
		defaultOwnerLoader.run.mockResolvedValue('skipped');
	...
	it('should run all loaders', async () => {
		...
		expect(defaultOwnerLoader.run).toHaveBeenCalled();
