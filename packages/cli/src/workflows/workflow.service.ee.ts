...
import { hasGlobalScope, type WorkflowSharingRole } from '@n8n/permissions';
...
export type WorkflowProjectShare = {
	projectId: string;
	role: Extract<WorkflowSharingRole, 'workflow:editor' | 'workflow:viewer'>;
};
...
export class EnterpriseWorkflowService {
	...
	async shareWithProjects(
		...
		const shareWithIds = shares.map(({ projectId }) => projectId);
		...
		const roleByProjectId = new Map(shares.map((share) => [share.projectId, share.role]));

		const newSharedWorkflows = projects.map((project) =>
			this.sharedWorkflowRepository.create({
				workflowId,
				role: roleByProjectId.get(project.id) ?? 'workflow:editor',
