...
import { EnterpriseWorkflowService, type WorkflowProjectShare } from './workflow.service.ee';
...
export class WorkflowsController {
	...
	async share(
		...
		const currentShares: WorkflowProjectShare[] = workflow.shared
			.filter((sharedWorkflow) => sharedWorkflow.role !== 'workflow:owner')
			.map((sharedWorkflow) => ({
				projectId: sharedWorkflow.projectId,
				role:
					sharedWorkflow.role === 'workflow:viewer' ? 'workflow:viewer' : 'workflow:editor',
			}));

		const currentShareMap = new Map(currentShares.map((share) => [share.projectId, share.role]));
		const targetShareMap = new Map(targetShares.map((share) => [share.projectId, share.role]));

		const toUnshare = [...currentShareMap.keys()].filter(
			(projectId) => !targetShareMap.has(projectId),
		);
		const toShare = targetShares.filter((share) => !currentShareMap.has(share.projectId));
		const toUpdateRole = targetShares.filter(
			(share) =>
				currentShareMap.has(share.projectId) &&
				currentShareMap.get(share.projectId) !== share.role,
		);

		if (toShare.length > 0 || toUpdateRole.length > 0) {
		...
		await dbManager.transaction(async (trx) => {
			...
			for (const share of toUpdateRole) {
				await trx.update(
					SharedWorkflow,
					{ workflowId, projectId: share.projectId },
					{ role: share.role },
				);
			}

			newShareeIds = toShare.map((share) => share.projectId);
		});

		this.eventService.emit('workflow-sharing-updated', {
			workflowId,
			userIdSharer: req.user.id,
			userIdList: targetShares.map((share) => share.projectId),
		});
	...
	private getWorkflowShareTargets(body: ShareWorkflowBodyDto): WorkflowProjectShare[] {
		...
		if (body.shareWithIds !== undefined) {
			if (!Array.isArray(body.shareWithIds) || !body.shareWithIds.every((id) => typeof id === 'string')) {
				throw new BadRequestError('Bad request');
			}

			return body.shareWithIds.map((projectId) => ({
				projectId,
