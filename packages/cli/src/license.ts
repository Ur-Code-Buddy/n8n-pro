...
/** Licensed = feature disabled or unwanted UI (negative flags). */
const NEGATIVE_LICENSE_FEATURES: ReadonlySet<BooleanLicenseFeature> = new Set([
	LICENSE_FEATURES.API_DISABLED,
	LICENSE_FEATURES.SHOW_NON_PROD_BANNER,
]);

const LICENSE_QUOTA_VALUES: ReadonlySet<string> = new Set(Object.values(LICENSE_QUOTAS));
...
export class License implements LicenseProvider {
	...
	isLicensed(feature: BooleanLicenseFeature) {
		if (this.globalConfig.license.unlockAllFeatures) {
			return !NEGATIVE_LICENSE_FEATURES.has(feature);
		}
	...
	getValue<T extends keyof FeatureReturnType>(feature: T): FeatureReturnType[T] {
		if (this.globalConfig.license.unlockAllFeatures) {
			...
			if (LICENSE_QUOTA_VALUES.has(feature as string)) {
