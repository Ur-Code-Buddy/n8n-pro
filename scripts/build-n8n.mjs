...
function getBuildResources() {
	...
	// Hypothesis B/E: low-RAM hosts need capped concurrency and explicit heap.
	const heapMb = process.env.N8N_BUILD_HEAP_MB
		? Number(process.env.N8N_BUILD_HEAP_MB)
		: totalMemMb <= 4096
			? 3072
			: totalMemMb <= 8192
				? 4096
	...
	const concurrency = process.env.N8N_BUILD_CONCURRENCY
		? Number(process.env.N8N_BUILD_CONCURRENCY)
		: totalMemMb <= 8192
			? 1
			: totalMemMb <= 16384
				? 2
	...
	return { totalMemMb, cpuCount, heapMb, concurrency, nodeOptions };
}

function getBuildEnv() {
	const resources = getBuildResources();
	return { ...process.env, NODE_OPTIONS: resources.nodeOptions, FORCE_COLOR: '1' };
}
...
async function runDeployOrContinue({ filter, targetDir, label }) {
	try {
		await $`cd ${config.rootDir} && NODE_ENV=production DOCKER_BUILD=true pnpm --config.strict-peer-dependencies=false --filter=${filter} --prod --legacy deploy --no-optional ${targetDir}`;
	} catch {
		const outputDir = path.isAbsolute(targetDir)
			? targetDir
			: path.join(config.rootDir, targetDir.replace(/^\.\//, ''));
		const hasOutput =
			(await fs.pathExists(path.join(outputDir, 'bin'))) ||
			(await fs.pathExists(path.join(outputDir, 'dist')));
		if (!hasOutput) {
			throw new Error(`${label} deploy failed and output directory is incomplete`);
		}
		echo(
			chalk.yellow(
				`WARN: ${label} deploy reported peer dependency issues but output exists; continuing`,
			),
		);
	}
}
...
await runDeployOrContinue({ filter: 'n8n', targetDir: './compiled', label: 'n8n' });
...
await runDeployOrContinue({
	filter: '@n8n/task-runner',
	targetDir: './dist/task-runner-javascript',
	label: 'task-runner',
});
